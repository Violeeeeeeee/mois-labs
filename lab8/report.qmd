...
4.  **Weryfikacja:** Poprawność wyniku sprawdzono poprzez obliczenie normy euklidesowej wektora residuum $r = ||Ax - b||$, wykorzystując operacje BLAS poziomu 2 dostępne w GSL[@gsl_blas, @blas_ops].
...
Zestawienie uzyskanych czasów wykonania oraz błędów numerycznych zaprezentowano w Tabeli \ref{tbl-results}. Wartości błędu residuum dla wszystkich prób oscylowały w granicach precyzji maszynowej ($10^{-9}$ do $10^{-14}$), co potwierdza poprawność i stabilność numeryczną zastosowanych metod[@gsl_linalg].
...
\hline
10 & 0.000024 & 0.000009 & $3.2171 \times 10^{-14}$ \\
100 & 0.000875 & 0.000049 & $4.7298 \times 10^{-12}$ \\
500 & 0.031188 & 0.000335 & $5.7555 \times 10^{-10}$ \\
600 & 0.057770 & 0.000595 & $8.4344 \times 10^{-11}$ \\
800 & 0.117500 & 0.000861 & $3.4065 \times 10^{-10}$ \\
1000 & 0.234349 & 0.001321 & $1.0893 \times 10^{-09}$ \\
\hline
\end{tabular}
\end{table}
```
...
![Zależność czasu dekompozycji LU od rozmiaru macierzy $N$. Widoczny trend wzrostu rzędu $O(N^3)$.](gsl_methods_error.pdf){#fig-decomp width=100% fig-pos='h'}
...
![Zależność czasu rozwiązywania układu od rozmiaru macierzy $N$. Trend zgodny ze złożonością $O(N^2)$.](hit_and_miss_error.pdf){#fig-solve width=100% fig-pos='h'}
...
1.  **Etap dekompozycji:**
    $$\frac{T_{decomp}(1000)}{T_{decomp}(500)} = \frac{0.234349}{0.031188} \approx 7.51$$
    Teoretycznie, dla złożoności sześciennej oczekiwany współczynnik wzrostu wynosi $2^3 = 8$. Uzyskany wynik (ok. 7.51) jest bardzo bliski wartości teoretycznej, co potwierdza sześcienną naturę algorytmu. Niewielkie odchylenie wynika prawdopodobnie z narzutów systemowych i zarządzania pamięcią, które mają relatywnie większy wpływ przy mniejszych wymiarach macierzy.

2.  **Etap rozwiązywania:**
    $$\frac{T_{solve}(1000)}{T_{solve}(500)} = \frac{0.001321}{0.000335} \approx 3.94$$
    Wynik ten jest niemal identyczny z wartością teoretyczną dla złożoności kwadratowej ($2^2 = 4$).
