...
\title{Sprawozdanie z laboratorium: Analiza złożoności obliczeniowej rozwiązywania układów równań liniowych metodą dekompozycji LU}
...
Celem niniejszego ćwiczenia była empiryczna weryfikacja teoretycznej złożoności obliczeniowej algorytmu rozwiązywania układów równań liniowych postaci $Ax = b$ z wykorzystaniem dekompozycji LU. Literatura przedmiotu wskazuje, że rozkład nieosobliwej macierzy $A$ na macierz dolnotrójkątną ($L$) i górnotrójkątną ($U$) wymaga wykonania około $\frac{2}{3}N^3$ operacji zmiennoprzecinkowych, co przekłada się na złożoność czasową rzędu $O(N^3)$[@num_algo]. Natomiast drugi etap procesu, czyli rozwiązanie układu metodą podstawiania (w przód i wstecz), charakteryzuje się złożonością $O(N^2)$.

W sprawozdaniu przedstawiono analizę wydajności obu etapów algorytmu przy użyciu biblioteki *GNU Scientific Library* (GSL)[@gsl_ref]. Pomiary czasu wykonania przeprowadzono dla macierzy o wymiarach z przedziału od $N=10$ do $N=1000$.

## Metodyka

Program testowy zaimplementowano w języku C, wykorzystując funkcje biblioteczne GSL: `gsl_linalg_LU_decomp` oraz `gsl_linalg_LU_solve`, zgodnie z dokumentacją[@gsl_ref]. Procedura badawcza obejmowała następujące kroki:

1.  **Generacja danych:** Dla każdego badanego wymiaru $N \in [10, 1000]$, macierz $A$ oraz wektor $b$ wypełniono losowymi wartościami o rozkładzie jednostajnym.
...
Zestawienie uzyskanych czasów wykonania oraz błędów numerycznych zaprezentowano w Tabeli \ref{tbl-results}. Wartości błędu residuum dla wszystkich prób oscylowały w granicach precyzji maszynowej ($10^{-10}$ do $10^{-13}$), co potwierdza poprawność i stabilność numeryczną zastosowanych metod[@gsl_ref].
...
Otrzymane wyniki jednoznacznie ukazują dysproporcję w tempie wzrostu kosztu obliczeniowego obu operacji. Na Rysunku \ref{fig-decomp} przedstawiono czas dekompozycji, który wykazuje charakterystyczny dla złożoności $O(N^3)$ trend sześcienny[@num_algo].
...
Przeprowadzone eksperymenty w pełni potwierdzają teoretyczne założenia dotyczące złożoności algorytmów algebry liniowej[@num_algo]. Koszt dekompozycji LU ($O(N^3)$) dominuje nad kosztem rozwiązania układu ($O(N^2)$), co staje się szczególnie widoczne wraz ze wzrostem wymiaru zadania.
